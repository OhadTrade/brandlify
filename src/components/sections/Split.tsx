...
export async function Split() {
  ...
  return (
    <section
      data-flow="lift" className="section-y bg-surface border-line border-y" aria-labelledby="split-heading">
      <Container>
        <div className="grid items-center gap-14 lg:grid-cols-2 lg:gap-16">
          <div className="flex flex-col gap-7">
            <h2 id="split-heading" className="text-h2 text-fg">
              אתר שלא רק נראה טוב. <span className="text-brand-gradient">אתר שמביא לקוחות.</span>
            </h2>
            <p className="text-muted text-[1.0625rem] leading-relaxed">{split.body}</p>
            ...
            <ul className="grid gap-4 sm:grid-cols-2">
              {split.points.map((point) => (
                <li key={point.title} className="flex gap-3">
                  ...
                  <span>
                    <span className="text-fg block font-semibold">{point.title}</span>
          ...
          <div className="relative mx-auto w-full max-w-lg" aria-hidden>
            ...
            <Parallax yPercent={-18} className="absolute -bottom-8 end-4 z-10 w-32 sm:w-36">
